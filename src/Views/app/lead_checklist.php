...
<section class="section">
  <div class="flex justify-between items-center gap-12 mb-12">
    <h2>Checklist / Estimate (Lead #<?= e((string)($lead['id'] ?? '')) ?>)</h2>
    <div class="flex gap-10 items-center flex-wrap">
      <a class="btn" href="<?= e(app_url($ctx, '/app/leads/' . (string)($lead['id'] ?? ''))) ?>">Back to lead</a>
  ...
  <div class="grid-2">
    <div class="card">
      ...
      <div class="muted mt-6"><?= e((string)($lead['address'] ?? '')) ?></div>
    </div>
    ...
    <div class="card card--stone">
      ...
      <?php if ($isDraft): ?>
        <form method="post" action="<?= e(app_url($ctx, '/app/leads/' . (string)($lead['id'] ?? '') . '/checklist/submit')) ?>" class="mt-12">
          <?= csrf_field($ctx) ?>
          <button class="btn btn--gold" type="submit">Submit to client</button>
          <div class="muted mt-6">Client will see it in Portal → Approvals.</div>
        </form>
      <?php else: ?>
        <div class="muted mt-12">This checklist is locked after submission.</div>
      <?php endif; ?>
    </div>
  </div>
</section>
...
<section class="section">
  ...
  <?php if ($isDraft): ?>
    <div class="card mb-14">
      ...
      <form method="post" action="<?= e(app_url($ctx, '/app/leads/' . (string)($lead['id'] ?? '') . '/checklist/items')) ?>" class="mt-10">
        <?= csrf_field($ctx) ?>
        <div class="row">
      ...
      <tbody>
        <?php foreach ($items as $it): ?>
          ...
          <tr>
            ...
            <td>
              <?php if ($isDraft): ?>
                <form method="post" action="<?= e(app_url($ctx, '/app/checklist-items/' . (string)$it['id'] . '/update')) ?>" class="flex gap-10 items-center flex-wrap">
                  <?= csrf_field($ctx) ?>
                ...
                <form method="post" action="<?= e(app_url($ctx, '/app/checklist-items/' . (string)$it['id'] . '/delete')) ?>" class="mt-6">
                  <?= csrf_field($ctx) ?>
